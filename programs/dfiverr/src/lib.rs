...
declare_id!("53f9PzxmMi2ztWzTvM1pNgJXhhxyuPNCMNk6y9DPfELZ");
...
// Helper function outside the #[program] macro
pub fn release_funds(ctx: Context<MarkCompleted>) -> Result<()> {
    let escrow = &mut ctx.accounts.escrow;
    require!(escrow.is_completed, ErrorCode::EscrowNotCompleted);
...
pub struct MarkCompleted<'info> {
    ...
    #[account(mut)]
    /// CHECK: This is the freelancer account
